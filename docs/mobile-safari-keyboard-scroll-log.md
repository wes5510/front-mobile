...
### 15. 키보드를 내린 뒤 Footer input이 눌리지 않는 WebKit 버그

- 증상: 키보드를 닫고 `window.scrollTo`로 문서 스크롤을 복구한 뒤, **Footer의
  input이 화면에 멀쩡히 보이는데 터치해도 포커스가 잡히지 않는다.** 레이아웃은
  정상이고 값도 정상인데 히트 테스트만 어긋나는, WebKit 엔진 버그로 보인다.
- 해결: 스크롤 복구 후 Footer 엘리먼트의 `visibility`를 `hidden` → `visible`로
  토글해 강제로 다시 그리게 하면 풀린다.
- 여기에 두 가지 타이밍 조건이 붙는다 (둘 다 실험으로 확인):
  - **토글까지 600ms를 기다려야 한다.** 스크롤 복구 직후나 한 프레임 뒤에
    토글하면 여전히 안 눌린다. 키보드 닫힘 애니메이션이 완전히 끝난 뒤라야
    다시 그리는 게 의미가 있는 것으로 보인다.
  - **`visible` 복구는 다음 프레임으로 미뤄야 한다.** 같은 태스크 안에서 껐다
    켜면 두 대입이 상쇄되어 브라우저가 실제로는 아무것도 다시 그리지 않는다.
    (`offsetHeight`를 읽어 강제 리플로우를 거는 방식으로도 시도해봤지만, 실제
    기기에서는 다음 프레임으로 나누는 쪽이 확실했다.)
